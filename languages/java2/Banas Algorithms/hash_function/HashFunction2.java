...
public class HashFunction2 {
	...
	public void moveOldArray(int newArraySize) {
		// Create an array that has allt the values of
	...
	public String[] removeEmptySpacesInArray(String[] arrayToClean) {
		...
		// Cycle through the array and if a space doesn't contain
		// -1, or isn't empty add it to the ArrayList
	...
	public void doubleHashFunc(String[] stringsForArray, String[] theArray) {
		for (int n = 0; n < stringsForArray.length; n++) {
			...
			// after a collision occurs. We are doing this rather than
			// just going to the next index to avoid creating clusters
	...
	// Return the value stored in the Double Hashed Hash Table
	...
	public String findKeyDblHashed(String key) {
		...
		// Find the key's original step distance
		int stepDistance = 5 - (Integer.parseInt(key) % 5);
		while (theArray[arrayIndexHash] != "-1") {
			if (theArray[arrayIndexHash] == key) {
				// Found thedey so return it
		...
		for (int m = 0; m < numberOfRows; m++) {
			...
			for (int n = increment - 10; n < increment; n++) {
				if (n >= arraySize)
					...
					// theArray[n]));
					System.out.format(String.format("| %3s " + " ", theArray[n]));
			}

			System.out.println("|"); // End of line deco
