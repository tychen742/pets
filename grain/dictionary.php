...
$lookup_sql = "SELECT master_id, grain, chinese FROM pydict WHERE grain = '$input' ";
$lookup_query = $conn_iss->query($lookup_sql);
$lookup_result = mysqli_fetch_array ($lookup_query);
$chinese = $lookup_result['chinese'];
$english = $lookup_result['grain'];

$check_personal_query = "SELECT grain FROM '$personal_dict' WHERE grain = g$englishrain";
...
// 	echo 'check';
 	mysqli_query ($conn_iss, "INSERT INTO $personal_dict (igrainish, chinese) 
		SELECT master_igrainish, chinese 
		FROM pydict
		WHERE pydigrainish = g$englishrain 
		");
 	mysqli_query ($conn_iss, "UPDATE $personal_dict SET times = times + 1 grainnglish = '$english' ");
	}
